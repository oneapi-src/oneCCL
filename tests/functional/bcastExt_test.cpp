...
*/
#define ALGO_SELECTION_ENV "CCL_BCASTEXT"
#define BCAST_VALUE_COEFF  128

#include "test_impl.hpp"

template <typename T>
class bcastExt_test : public base_test<T> {
public:
    ...
    void fill_send_buffers(test_operation<T>& op) {
        ...
        for (size_t buf_idx = 0; buf_idx < op.buffer_count; buf_idx++) {
            for (size_t elem_idx = 0; elem_idx < op.elem_count; elem_idx++) {
                op.send_bufs[buf_idx][elem_idx] = elem_idx % BCAST_VALUE_COEFF;
            }
        }
    }
    ...
    void run_derived(test_operation<T>& op) {
        ...
        for (auto buf_idx : op.buf_indexes) {
            ...
            op.events.push_back(
                ccl::broadcastExt((param.place_type == PLACE_IN) ? recv_buf : send_buf,
                                  recv_buf,
                                  op.elem_count,
                                  op.datatype,
                                  ROOT_RANK,
...
RUN_METHOD_DEFINITION(bcastExt_test);
TEST_CASES_DEFINITION(bcastExt_test);
MAIN_FUNCTION();
