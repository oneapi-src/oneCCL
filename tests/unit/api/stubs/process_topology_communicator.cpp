...
template <class buffer_type>
ccl::request_t process_a2a_communicator::allgatherv_impl(
    const buffer_type& send_buf,
    size_t send_count,
...
template <class buffer_type>
ccl::request_t process_a2a_communicator::alltoall_impl(
...
/* reduce_scatter */
template <class buffer_type>
ccl::coll_request_t process_a2a_communicator::reduce_scatter_impl(
    const buffer_type* send_buf,
    buffer_type* recv_buf,
    size_t recv_count,
    ccl::reduction reduction,
...
ccl::request_t process_ring_communicator::barrier(ccl::stream::impl_value_t& stream,
                                                  const ccl::barrier_attr& attr,
                                                  const ccl::vector_class<ccl::event>& deps) {
    return {};
}
...
/* allgatherv */
ccl::coll_request_t process_ring_communicator::allgatherv_impl(
    const void* send_buf,
    size_t send_count,
    void* recv_buf,
    const ccl::vector_class<size_t>& recv_counts,
...
/* allreduce */
ccl::coll_request_t process_ring_communicator::allreduce_impl(
    const void* send_buf,
    void* recv_buf,
    size_t count,
    ccl::datatype dtype,
...
/* alltoall */
ccl::coll_request_t process_ring_communicator::alltoall_impl(
    const void* send_buf,
    void* recv_buf,
    size_t count,
    ccl::datatype dtype,
...
/* alltoallv */
ccl::coll_request_t process_ring_communicator::alltoallv_impl(
    const void* send_buf,
    const ccl::vector_class<size_t>& send_counts,
    void* recv_buf,
    const ccl::vector_class<size_t>& recv_counts,
...
/* bcast */
ccl::coll_request_t process_ring_communicator::broadcast_impl(
    void* buf,
    size_t count,
    ccl::datatype dtype,
    size_t root,
...
/* reduce */
ccl::coll_request_t process_ring_communicator::reduce_impl(
    const void* send_buf,
    void* recv_buf,
    size_t count,
    ccl::datatype dtype,
...
/* reduce_scatter */
ccl::request_t process_ring_communicator::reduce_scatter_impl(
    const void* send_buf,
    void* recv_buf,
    size_t recv_count,
    ccl::datatype dtype,
...
/* sparse_allreduce */
ccl::coll_request_t process_ring_communicator::sparse_allreduce_impl(
    const void* send_ind_buf,
    size_t send_ind_count,
    const void* send_val_buf,
    size_t send_val_count,
...
template <class buffer_type>
ccl::request_t process_ring_communicator::allgatherv_impl(
    const buffer_type& send_buf,
    size_t send_count,
    ccl::vector_class<ccl::reference_wrapper_class<buffer_type>>& recv_buf,
    const ccl::vector_class<size_t>& recv_counts,
...
template <class buffer_type>
ccl::request_t process_ring_communicator::alltoall_impl(
    const ccl::vector_class<buffer_type*>& send_buf,
    const ccl::vector_class<buffer_type*>& recv_buf,
    size_t count,
    ccl::stream::impl_value_t& stream,
...
/* reduce_scatter */
template <class buffer_type>
ccl::coll_request_t process_ring_communicator::reduce_scatter_impl(
    const buffer_type* send_buf,
    buffer_type* recv_buf,
    size_t recv_count,
    ccl::reduction reduction,
