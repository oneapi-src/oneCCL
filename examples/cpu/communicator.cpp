...
void check_comm_create_colored() {
    ...
    for (size_t split_by = 2; split_by <= size; split_by *= 2) {
        ...
        if (comm_size != expected_ranks_count) {
            ...
            throw runtime_error("mismatch in size, expected " + to_string(expected_ranks_count) +
        ...
        if (comm_size_inside != expected_ranks_count_inside) {
            ...
            throw runtime_error("mismatch in size, expected " +
                                to_string(expected_ranks_count_inside) + " received " +
                                to_string(comm_size_inside));
        }

        PRINT_BY_ROOT(
            "global comm: idx=%zu, count=%zu; new comms: rank=%zu, size=%zu; rank_inside=%zu, size_inside=%zu\n",
            ::rank,
            size,
            comm_rank,
            comm_size,
...
void check_comm_create_identical_color() {
    ...
    if (comm_size != size) {
        ...
        printf("FAILED\n");
        throw runtime_error("mismatch in size, expected " + to_string(size) + " received " +
                            to_string(comm_size));
    }
    ...
    if (comm_rank != ::rank) {
        ...
        printf("FAILED\n");
        throw runtime_error("mismatch in rank, expected " + to_string(::rank) + " received " +
                            to_string(comm_rank));
    }

    PRINT_BY_ROOT("global comm: rank=%zu, size=%zu; new comm: rank=%zu, size=%zu",
                  ::rank,
                  size,
                  comm_rank,
                  comm_size);
