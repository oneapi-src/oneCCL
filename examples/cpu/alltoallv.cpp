...
int main()
{
    ...
    ASSERT((EVEN_RANK_SEND_COUNT + ODD_RANK_SEND_COUNT) % 2 == 0,
           ...
           EVEN_RANK_SEND_COUNT, ODD_RANK_SEND_COUNT);

    ASSERT(size % 2 == 0,
           "unexpected rank count (%zu)", size);
    ...
    RUN_COLLECTIVE(ccl_alltoallv(send_buf, send_counts, recv_buf, recv_counts,
                                 ccl_dtype_int, &coll_attr, NULL, NULL, &request),
                   "warmup_alltoallv");
