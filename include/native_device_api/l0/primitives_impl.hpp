...
void memory<TEMPLATE_DEF_ARG>::enqueue_write_sync(const std::vector<elem_t>& src)
{
    if (count() < src.size())
    {
        throw std::length_error(std::string(__PRETTY_FUNCTION__) +
    ...
    try
    {
        detail::copy_memory_to_device_sync_unsafe(get(), src.data(), src.size() * sizeof(elem_t),
                                                get_owner());
    }
...
template<TEMPLATE_DECL_ARG>
void memory<TEMPLATE_DEF_ARG>::enqueue_write_sync(typename std::vector<elem_t>::const_iterator first,
                                                  typename std::vector<elem_t>::const_iterator last)
{
    ...
    try
    {
        detail::copy_memory_to_device_sync_unsafe(get(), &(*first), requested_size,
                                                get_owner());
    }
    catch(const std::exception& ex)
    {
...
void memory<TEMPLATE_DEF_ARG>::enqueue_write_sync(const std::array<elem_t, N>& src)
{
    ...
    try
    {
        detail::copy_memory_to_device_sync_unsafe(get(), src.data(), N * sizeof(elem_t), get_owner());
    }
    catch(const std::exception& ex)
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + "\n" + ex.what());
...
void memory<TEMPLATE_DEF_ARG>::enqueue_write_sync(const elem_t* src, size_t src_elem_count)
{
    ...
    try
    {
        detail::copy_memory_to_device_sync_unsafe(get(), src, src_elem_count * sizeof(elem_t), get_owner());
    }
    catch(const std::exception& ex)
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + "\n" + ex.what());
...
template<TEMPLATE_DECL_ARG>
std::vector<elem_t> memory<TEMPLATE_DEF_ARG>::enqueue_read_sync(size_t src_elem_count/* = 0*/) const
{
    if (src_elem_count == 0)
    {
    ...
    try
    {
        ...
        detail::copy_memory_to_device_sync_unsafe(dst.data(), get(), actual_size * sizeof(elem_t), get_owner());
    }
    catch(const std::exception& ex)
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + "\n" + ex.what());
