...
#define INLINE_TARGET_ATTRIBUTE_ALL \
    __attribute__((__always_inline__, target(ALL_BFP16_ATTRS))) inline
...
INLINE_TARGET_ATTRIBUTE_BW void ccl_fp32_store_as_bfp16_avx512f(const void* src, void* dst) {
    _mm256_storeu_si256((__m256i*)(dst),
                        _mm512_cvtepi32_epi16(_mm512_bsrli_epi128(_mm512_loadu_si512(src), 2)));
}

#ifdef CCL_BFP16_AVX512BF_COMPILER
INLINE_TARGET_ATTRIBUTE_BF16 void ccl_fp32_store_as_bfp16_avx512bf(const void* src, void* dst) {
...
\
    INLINE_TARGET_ATTRIBUTE_ALL void ccl_bfp16_reduce_inputs_##impl_type( \
        const void* a, const void* b, void* res, ccl_bfp16_reduction_func_ptr op) { \
...
\
    INLINE_TARGET_ATTRIBUTE_ALL void ccl_bfp16_reduce_256_##impl_type( \
        const void* in, const void* inout, ccl_bfp16_reduction_func_ptr op) { \
        __m256i vbfp16_out; \
        ccl_bfp16_reduce_inputs_##impl_type(in, inout, (void*)&vbfp16_out, op); \
        _mm256_storeu_si256((__m256i*)(inout), vbfp16_out); \
    } \
\
    INLINE_TARGET_ATTRIBUTE_ALL void ccl_bfp16_reduce_masked_##impl_type( \
        const void* in, void* inout, uint8_t len, ccl_bfp16_reduction_func_ptr op) { \
        if (len == 0) \
            return; \
...
\
    INLINE_TARGET_ATTRIBUTE_ALL void ccl_bfp16_reduce_impl_##impl_type( \
        const void* in_buf, void* inout_buf, size_t in_cnt, ccl_bfp16_reduction_func_ptr op) { \
        int i = 0; \
        for (i = 0; i <= (int)in_cnt - 16; i += 16) { \
            ccl_bfp16_reduce_256_##impl_type((uint16_t*)in_buf + i, (uint16_t*)inout_buf + i, op); \
        } \
        ccl_bfp16_reduce_masked_##impl_type( \
...
INLINE_TARGET_ATTRIBUTE_ALL void ccl_bfp16_reduce_impl(const void* in_buf,
                                                       void* inout_buf,
                                                       size_t in_cnt,
