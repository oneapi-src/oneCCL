...
namespace detail
{
    CCL_API void copy_memory_to_device_sync_unsafe(void* dst, const void* src, size_t size,
                                                   std::weak_ptr<ccl_device> device_weak)
    {
        ...
        ze_result_t ret = zeCommandListAppendMemoryCopy(cmd_list.get(),
                                                        dst, src,
                                                        size, nullptr);
        ...
        if(ret != ZE_RESULT_SUCCESS )
        {
            throw std::runtime_error(std::string("Cannot reset queue, error: ") +
                                     std::to_string(ret));
        }
    }
}
...
std::string get_build_log_string(const ze_module_build_log_handle_t &build_log)
{
    ...
    {
        throw std::runtime_error("xeModuleBuildLogGetString failed: " +
                                 to_string(result));
    }

    std::vector<char> build_log_c_string(build_log_size);
    result = zeModuleBuildLogGetString(build_log, &build_log_size,
...
bool command_queue_desc_comparator::operator() (const ze_command_queue_desc_t& lhs, const ze_command_queue_desc_t& rhs) const
{
    return memcmp(&lhs, &rhs, sizeof(rhs)) < 0;
}

bool command_list_desc_comparator::operator() (const ze_command_list_desc_t& lhs, const ze_command_list_desc_t& rhs) const
{
     return memcmp(&lhs, &rhs, sizeof(rhs)) < 0;
}
}
