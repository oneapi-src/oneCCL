...
CCL_API
std::shared_ptr<ccl_device> ccl_device::create(handle_t handle, owner_ptr_t&& driver,
                                               const ccl::device_indices_t& indexes/* = ccl::device_indices_t()*/)
{
    // TODO - dirty code
    owner_ptr_t shared_driver(std::move(driver));
    std::shared_ptr<ccl_device> device = std::make_shared<ccl_device>(handle,
                                                                      shared_driver.lock()->get_ptr());
    ...
    try
    {
        for (const auto &val : collected_subdevices_list)
        {
            if(indexes.empty())
            {
                device->sub_devices.emplace(val.first,
                                            ccl_subdevice::create(val.second,
                                                                  device->get_ptr(),
                                                                  shared_driver.lock()->get_ptr()));
            }
            else
            {
                ...
                for(const auto& affitinity : indexes)
                {
                    ...
                    if(std::get<ccl::device_index_enum::subdevice_index_id>(affitinity) == val.first)
                    {
                        device->sub_devices.emplace(val.first,
                                                    ccl_subdevice::create(val.second,
                                                                          device->get_ptr(),
...
CCL_API
ccl_device::indexed_handles ccl_device::get_handles(const ccl_device_driver& driver,
                                                    const ccl::device_indices_t& requested_device_indexes/* = indices()*/)
{
    ...
    {
        throw std::runtime_error(std::string(__FUNCTION__) +
                                 " - zeDeviceGet failed for device request, error: " +
                                 native::to_string(err));
    }
    ...
    {
        ret = 
            detail::collect_indexed_data<ccl::device_index_enum::device_index_id>(
                                        filtered_ids,
                                        handles,
                                        std::bind(get_device_properties_from_handle,
                                                   std::placeholders::_1));
    ...
    {
        throw std::runtime_error(std::string(__FUNCTION__) + " - Cannot add device: " +
                                 ex.what());
    }
    return ret;
}

ccl_device::ccl_device(handle_t h, owner_ptr_t&& parent) :
    base(h, std::move(parent))
{
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("zeDeviceGetMemoryProperties failed for nullptr, error: ") + native::to_string(ret));
    }
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("zeDeviceGetMemoryProperties failed for memory_properties, error: ") + native::to_string(ret));
    }
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("zeDeviceGetMemoryAccessProperties failed for memory_access_properties, error: ") + native::to_string(ret));
    }
    ...
    if (ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("zeDeviceGetComputeProperties failed, error: ") + native::to_string(ret));
    }
}
...
{
    return const_cast<sub_devices_container_type& >(static_cast<const ccl_device* >(this)->get_subdevices());
}
...
{
    return std::const_pointer_cast<ccl_subdevice>(static_cast<const ccl_device* >(this)->get_subdevice(path));
}

CCL_API ccl_device::const_subdevice_ptr ccl_device::get_subdevice(const ccl::device_index_type& path) const
{
    ...
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + " - incorrect driver, expected: " +
                                 std::to_string(driver->get_driver_id()) + ", requested: " +
                                 ccl::to_string(path));
    }
    ...
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + " - incorrect device, expected: " +
                                 std::to_string(device_index) + ", requested: " + ccl::to_string(path));
    }
    ...
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + " - incorrect subdevice requested: " +
                                 ccl::to_string(path));
    }
    ...
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + " - incorrect subdevice index requested: " +
                                 ccl::to_string(path) + ". Total subdevices count: " + std::to_string(sub_devices.size()));
    }
...
CCL_API ccl::device_index_type ccl_device::get_device_path() const
{
    ...
    ccl::device_index_type device_path = std::make_tuple(driver->get_driver_id(),
                                                         get_device_id(),
                                                         std::numeric_limits<uint32_t>::max());
    return device_path;
}

CCL_API ze_device_p2p_properties_t ccl_device::get_p2p_properties(const ccl_device& remote_device) const
{
    ...
    if (ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error( std::string("Cannot execute zeDeviceGetP2PProperties, error: ") + native::to_string(ret));
    }
...
CCL_API ccl_device::device_queue ccl_device::create_cmd_queue(const ze_command_queue_desc_t &properties/* = get_default_queue_desc()*/)
{
    ze_command_queue_handle_t hCommandQueue;
    ze_result_t ret = zeCommandQueueCreate(handle, &properties, &hCommandQueue);
    if(ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("cannot allocate queue, error: ") + native::to_string(ret));
    }
...
CCL_API void* ccl_device::device_alloc_memory(size_t bytes_count, size_t alignment,
                                              const ze_device_mem_alloc_desc_t& mem_descr)
{
    void* out_ptr = nullptr;
    ze_result_t ret = //zeDriverAllocSharedMem(get_owner()->handle, handle, flags, ordinal, ZE_HOST_MEM_ALLOC_FLAG_DEFAULT, bytes_count, alignment, &out_ptr);
    //zeDriverAllocHostMem(get_owner()->handle, ZE_HOST_MEM_ALLOC_FLAG_DEFAULT, bytes_count, alignment, &out_ptr);
    zeDriverAllocDeviceMem(get_owner().lock()->handle, &mem_descr, bytes_count, alignment, handle, &out_ptr);
    if(ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("cannot allocate memory, error: ") + std::to_string(ret));
    }

    return out_ptr;
}

CCL_API ccl_device::handle_t ccl_device::get_assoc_device_handle(const void* ptr, const ccl_device_driver* driver)
{
    ...
    ze_result_t result = zeDriverGetMemAllocProperties(driver->handle, ptr, &mem_prop, &alloc_device_handle);
...
CCL_API ccl_device::device_ipc_memory_handle ccl_device::create_ipc_memory_handle(void* device_mem_ptr)
{
    ze_ipc_mem_handle_t ipc_handle;
    ze_result_t ret = zeDriverGetMemIpcHandle(get_owner().lock()->handle, device_mem_ptr, &ipc_handle); //TODO -- use device handle?
    if(ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("cannot get ipc mem handle, error: ") + native::to_string(ret));
    }
    return device_ipc_memory_handle(ipc_handle, get_ptr());
}

CCL_API std::shared_ptr<ccl_device::device_ipc_memory_handle> ccl_device::create_shared_ipc_memory_handle(void* device_mem_ptr)
{
    ze_ipc_mem_handle_t ipc_handle;
    //TODO thread-safety
    ...
    return ipc_storage.insert({device_mem_ptr, std::shared_ptr<device_ipc_memory_handle>( new device_ipc_memory_handle(ipc_handle, get_ptr()))}).first->second;
}
...
CCL_API ccl_device::device_ipc_memory ccl_device::get_ipc_memory(std::shared_ptr<device_ipc_memory_handle>&& ipc_handle)
{
    ...
    ze_result_t ret = zeDriverOpenMemIpcHandle(get_owner().lock()->handle, handle, ipc_handle->handle, flag, &(ipc_memory.pointer));
    if(ret != ZE_RESULT_SUCCESS)
    {
...
CCL_API std::shared_ptr<ccl_device::device_ipc_memory> ccl_device::restore_shared_ipc_memory(std::shared_ptr<device_ipc_memory_handle>&& ipc_handle)
{
    assert(ipc_handle->get_owner().lock().get() == this && "IPC handle doesn't belong to device: ");
    ze_ipc_memory_flag_t flag = ZE_IPC_MEMORY_FLAG_NONE;
    ip_memory_elem_t ipc_memory{};
...
void CCL_API ccl_device::on_delete(ip_memory_elem_t& ipc_mem)
{
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("cannot close ipc mem handle, error: ") + native::to_string(ret));
    }
}

CCL_API ccl_device::device_queue& ccl_device::get_cmd_queue(const ze_command_queue_desc_t &properties)
{
    if(properties.flags & ZE_COMMAND_QUEUE_FLAG_LOGICAL_ONLY)
    {
        ...
        if(properties.ordinal >= limit)
        {
            throw std::runtime_error(std::string("cannot requested queue \"ordinal\": ") +
                                     std::to_string(properties.ordinal) + " is greater than limit:" +
                                     std::to_string(limit));
        }
    }
...
ccl_device::device_cmd_list CCL_API ccl_device::create_cmd_list(const ze_command_list_desc_t &properties)
{
    ...
    if(ret != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("cannot allocate command list, error: ") + native::to_string(ret));
    }
    return device_cmd_list(hCommandList, get_ptr());
}

CCL_API ccl_device::device_cmd_list& ccl_device::get_cmd_list(const ze_command_list_desc_t &properties/* = get_default_list_desc()*/)
{
...
CCL_API ccl_device::device_module ccl_device::create_module(const ze_module_desc_t &descr)
{
    ...
    if (result != ZE_RESULT_SUCCESS)
    {
        ...
        throw std::runtime_error("zeModuleCreate failed: " + native::to_string(result) + ", Log: " + build_log_string);
    }

    result = zeModuleBuildLogDestroy(build_log);
    if (result)
    {
        throw std::runtime_error("zeModuleBuildLogDestroy failed: " +
                             native::to_string(result));
    }
    return device_module(module, get_ptr());
}
...
void CCL_API ccl_device::on_delete(ze_command_queue_handle_t& handle)
{
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("cannot destroy queue, error: ") + native::to_string(ret));
    }
...
void CCL_API ccl_device::on_delete(ze_command_list_handle_t& handle)
{
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("cannot destroy cmd list, error: ") + native::to_string(ret));
    }
...
void CCL_API ccl_device::on_delete(ze_device_handle_t& sub_device_handle)
{
    ...
    {
        throw std::runtime_error(std::string("cannot destroy subdevice handle: orphant subddevice: ") +
...
void CCL_API ccl_device::on_delete(ze_module_handle_t& module_handle)
{
    ...
    if(ret != ZE_RESULT_SUCCESS)
    {
        throw std::runtime_error(std::string("cannot destroy module handle, error: ") + native::to_string(ret));
    }
}

size_t ccl_device::serialize(std::vector<uint8_t> &out, size_t from_pos, size_t expected_size) const
{
    ...
    size_t serialized_bytes = driver->serialize(out, from_pos, expected_device_bytes + expected_size); //resize vector inside
...
std::weak_ptr<ccl_device> ccl_device::deserialize(const uint8_t** data, size_t& size, ccl_device_platform& platform)
{
    //restore driver
    ...
    decltype(device_properties.deviceId) recovered_index = *(reinterpret_cast<const decltype(device_properties.deviceId)*>(*data));
    size -= expected_bytes;
    *data+= expected_bytes;

    //find device with requested handle
    ccl::device_index_type path {driver->get_driver_id(), recovered_index, ccl::unused_index_value};
    auto device_ptr = driver->get_device(path);
    if (!device_ptr)
    {
        throw std::runtime_error(std::string(__FUNCTION__) + " - invalid device index: "
                                 + ccl::to_string(path));
    }
    return device_ptr;
}

std::string CCL_API ccl_device::to_string() const
{
    std::stringstream ss;
    ss << "handle: " << handle << ",\n------DeviceProperties:------\n" << native::to_string(device_properties)
       << "\n------MemoryProp:------\n";

    for(auto it = memory_properties.begin(); it != memory_properties.end(); ++it)
    {
        ss << native::to_string(*it) << ",\n";
    }

    ss << "\n------MemoryAccessProp:--\n";
    ss << native::to_string(memory_access_properties);
    ss << "\n------ComputeProp:------\n" << native::to_string(compute_properties) ;
    return ss.str();
}
