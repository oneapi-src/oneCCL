...
CCL_API std::shared_ptr<ccl_device_platform> ccl_device_platform::create(const ccl::device_indices_t& indices/* = device_indices_per_driver()*/)
{
...
*/
CCL_API void ccl_device_platform::init_drivers(const ccl::device_indices_t& driver_device_affinities/* = device_indices_per_driver()*/)
{
...
*/
    ...
    try
    {
        ...
        for (const auto &val : collected_drivers_list)
        {
            ...
            {
                drivers.emplace(val.first, ccl_device_driver::create(val.second,
                                                                     val.first,
                                                                     get_ptr()));
            }
            else
            {
                ...
                {
                    if(std::get<ccl::device_index_enum::driver_index_id>(affitinity) == val.first)
                    {
                ...
                drivers.emplace(val.first,
                                ccl_device_driver::create(val.second,
                                                          val.first,
                                                          get_ptr(), per_driver_index));
            }
        }
    }
...
CCL_API ccl_device_platform::const_driver_ptr ccl_device_platform::get_driver(ccl::index_type index) const
{
    auto it = drivers.find(index);
    if(it == drivers.end())
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + "No driver by index: " +
                                 std::to_string(index) + " on platform");
    }
    return it->second;
}
...
CCL_API ccl_device_driver::device_ptr ccl_device_platform::get_device(const ccl::device_index_type& path)
{
    return std::const_pointer_cast<ccl_device>(static_cast<const ccl_device_platform* >(this)->get_device(path));
}

CCL_API ccl_device_driver::const_device_ptr ccl_device_platform::get_device(const ccl::device_index_type& path) const
{
    ccl::index_type driver_idx = std::get<ccl::device_index_enum::driver_index_id>(path);
    auto it = drivers.find(driver_idx);
    if (it == drivers.end())
    {
        throw std::runtime_error(std::string(__PRETTY_FUNCTION__) + " - incorrect driver requested: " +
                                 ccl::to_string(path) + ". Total driver count: " + std::to_string(drivers.size()));
    }
