...
ccl_device_driver::indexed_driver_handles ccl_device_driver::get_handles(const ccl::device_indices_t& requested_driver_indexes/* = indices()*/)
{
    ...
    {
        throw std::runtime_error(std::string("zeDriverGet failed for count request, error: ") + native::to_string(err));
    }
    ...
    if(err != ZE_RESULT_SUCCESS )
    {
        throw std::runtime_error(std::string("zeDriverGet failed fro drivers request, error: ") + native::to_string(err));
    }
    ...
    {
        throw std::runtime_error(std::string("Cannot add driver: ") +
                                 ex.what());
    }
    return ret;
}

CCL_API std::shared_ptr<ccl_device_driver> ccl_device_driver::create(handle_t h,
                                                                     uint32_t id,
                                                                     owner_ptr_t&& platform,
                                                                     const ccl::device_mask_t& rank_device_affinity)
{
    std::shared_ptr<ccl_device_driver> driver = std::make_shared<ccl_device_driver>(h, id, std::move(platform));

    auto collected_devices_list = ccl_device::get_handles(*driver, get_device_indices(rank_device_affinity));
                                                                     ...
                                                                     const ccl::device_indices_t& rank_device_affinity/* = ccl::device_indices_t()*/)
{
    ...
    auto collected_devices_list = ccl_device::get_handles(*driver,
                                                          rank_device_affinity);
    try
    {
        for (const auto &val : collected_devices_list)
        {
            ...
            else
            {
                ...
                {
                    if(std::get<ccl::device_index_enum::device_index_id>(affitinity) == val.first)
                    {
...
CCL_API ccl_device_driver::device_ptr ccl_device_driver::get_device(const ccl::device_index_type& path)
{
    return std::const_pointer_cast<ccl_device>(static_cast<const ccl_device_driver* >(this)->get_device(path));
}

CCL_API ccl_device_driver::const_device_ptr ccl_device_driver::get_device(const ccl::device_index_type& path) const
{
...
CCL_API ccl::device_mask_t ccl_device_driver::create_device_mask(const std::string& str_mask, std::ios_base::fmtflags flag/* = std::ios_base::hex*/)
{
    std::stringstream ss;
    ss << str_mask;
...
CCL_API ccl::device_indices_t ccl_device_driver::get_device_indices(const ccl::device_mask_t& mask)
{
...
CCL_API ccl::device_mask_t ccl_device_driver::get_device_mask(const ccl::device_indices_t& device_idx)
{
...
std::weak_ptr<ccl_device_driver> ccl_device_driver::deserialize(const uint8_t** data, size_t& size, ccl_device_platform& platform)
{
    ...
    *data += expected_bytes;
    ...
    if (!driver_ptr)
    {
        throw std::runtime_error(std::string(__FUNCTION__) + " - invalid driver index: "
                                 + std::to_string(recovered_index));
    }
    return driver_ptr;
}

size_t ccl_device_driver::serialize(std::vector<uint8_t> &out, size_t from_pos, size_t expected_size) const
{
    ...
    assert(get_owner().lock()->get_drivers().size() == 1 && "Platform supports only one instance of the driver");
