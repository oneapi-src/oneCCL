...
CCL_API ccl_device_driver::device_ptr get_runtime_device(const DeviceType& device)
{
    ...
    if (!device.is_gpu())
    {
        throw std::runtime_error(std::string("get_runtime_device failed for sycl device: it is not gpu!"));
    }
...
template
native::ccl_device_driver::device_ptr
native::get_runtime_device(const ccl::device_index_type& path);

#ifdef CCL_ENABLE_SYCL
template
native::ccl_device_driver::device_ptr
native::get_runtime_device(const cl::sycl::device& device);
#endif
