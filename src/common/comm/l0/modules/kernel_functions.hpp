...
struct kernel_data_storage
{
    ...
    // generic setter
    template<class kernel_argument,
             class = typename std::enable_if<not std::is_pointer<typename kernel_argument::arg_type>::value>::type>
    ...
    template<class kernel_argument,
             class = typename std::enable_if<std::is_pointer<typename kernel_argument::arg_type>::value>::type>
    void set_arg(typename kernel_argument::arg_type new_val)
    {
        std::get<kernel_argument::index>(args).store(new_val);
    }
                                                     ...
                                                     arguments...>
{
    ...
    {
        ze_result_t result = zeKernelSetArgumentValue(handle, main_kernel_args::rank_index, sizeof(rank_type), &rank);
    ...
    {
        ze_result_t result = zeKernelSetArgumentValue(handle, main_kernel_args::size_index, sizeof(size_type), &size);
        if(result != ZE_RESULT_SUCCESS )
        {
    ...
    void set_arg(typename kernel_argument::arg_type& new_val)
    {
        LOG_TRACE("Function: ", name(), ", handle: ", handle, " - set_arg() index: ", kernel_argument::index, ", value: ", new_val);
        ...
        if(result != ZE_RESULT_SUCCESS )
        {
            CCL_THROW("Cannot set kernel argument by index: ", kernel_argument::index, ", kernel: ", name(), ", handle: ", handle);
        }
    ...
    void set_arg(typename kernel_argument::arg_type new_val)
    {
        LOG_TRACE("Function: ", name(), ", handle: ", handle, " - set_arg(pointer) index: ", kernel_argument::index, ", value: ", new_val);
        ze_result_t result = zeKernelSetArgumentValue(handle,
                                                      kernel_argument::index,
                                                      sizeof(typename kernel_argument::arg_type),
        ...
        if(result != ZE_RESULT_SUCCESS )
        {
            CCL_THROW("Cannot set kernel argument by pointer index: ", kernel_argument::index, ", kernel: ", name(), ", handle: ", handle);
        }
