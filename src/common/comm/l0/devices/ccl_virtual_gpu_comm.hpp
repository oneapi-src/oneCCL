...
class ccl_virtual_gpu_comm : public ccl_gpu_base_comm<ccl_virtual_gpu_comm,
                                                      gpu_types::VIRTUAL_GPU>,
                             public module_loader<ccl_virtual_gpu_comm>
{
public:
    using base = ccl_gpu_base_comm<ccl_virtual_gpu_comm,
                                   gpu_types::VIRTUAL_GPU>;
    using base::comm_rank_t;

    template<ccl_coll_type algo_type,
             ccl::device_topology_type topology_type>
    using gpu_module_t = virtual_gpu_coll_module<algo_type, topology_type>;
    ...
    using supported_modules = supported_device_modules<virtual_gpu_coll_module>;
    ...
    topology_addr<topology_type> get_real_comm_data() const
    {
        return real_gpu_comm.get_comm_data<topology_type>();
    }
    ...
    {
        const topology_addr<topology_type> &comm_addr = get_comm_data<topology_type>();
    ...
    std::string create_module_impl(const ze_module_desc_t &module_data)
    {
        ...
        auto real_kernel = real_gpu_comm.get_gpu_module_ptr<module_type, topology_type>();

        std::get<topology_type>(std::get<module_type>(registered_modules)).reset(new gpu_module_t<module_type, topology_type>(real_kernel));
