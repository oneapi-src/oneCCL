...
template <class managed_kernel,
          class entry,
          class ...binded_kernels>
struct kernel_connector : public base_connector_interface<managed_kernel>
{
    ...
    {
        return connect_impl(kernel_to_connect,
                            typename sequence_generator<std::tuple_size<binding_type>::value>::type());
    }
private:
    template<int ...connected_arguments_indices>
    bool connect_impl(managed_kernel& kernel_to_connect, numeric_sequence<connected_arguments_indices ...>)
    {
