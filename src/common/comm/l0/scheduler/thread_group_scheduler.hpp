...
struct thread_group_scheduler
{
  ...
  */
    thread_group_scheduler(size_t threads_count) :
      thread_group_size(threads_count)
    {
    ...
    thread_schedule_ptr submit_entry(size_t thread_id, device_community<topology>& device_topology, device_t& device, Arguments &&...args)
    {
        ...
        LOG_DEBUG("Thread id: ", thread_id, " device_group_size: ", device_group_size, " comm data: ", comm_data.to_string());
