...
ccl_gpu_concurrent_sched::ccl_gpu_concurrent_sched(size_t expected_threads_count,
...
std::shared_ptr<ccl_gpu_sched> ccl_gpu_concurrent_sched::get_gpu_sched(size_t thread_id)
{
    ...
    {
        LOG_ERROR("Requested thread id: ", thread_id, " is more than expected: ", partial_scheds.size());
    }
    return partial_scheds[thread_id];
}

std::shared_ptr<ccl_gpu_sched> ccl_gpu_concurrent_sched::create_gpu_sched(size_t thread_id,
...
ccl_gpu_concurrent_sched::ccl_gpu_concurrent_sched_ptr ccl_gpu_concurrent_sched::create(size_t thread_count, const ccl_coll_param& param/* = ccl_coll_param()*/)
{
    /*TODO use cache*/
