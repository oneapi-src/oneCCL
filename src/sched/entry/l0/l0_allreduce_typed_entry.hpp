...
public:
    ...
    using income_data_flag_gpu_type = typename std::remove_pointer<typename kernel_main_typed::income_data_flag_arg_type>::type;
    using ready_to_recv_flag_gpu_type = typename std::remove_pointer<typename kernel_main_typed::ready_to_recv_flag_arg_type>::type;
    using local_barrier_flag_gpu_type = typename std::remove_pointer<typename kernel_main_typed::local_barrier_flag_arg_type>::type;
    ...
    l0_allreduce_typed_entry(ccl_sched* sched,
        ...
        temp_buffer(parent_communicator->get_device().template alloc_memory<native_type>(cnt, sizeof(native_type))),
        income_data_flag(parent_communicator->get_device().template alloc_memory<income_data_flag_gpu_type>(1, sizeof(income_data_flag_gpu_type))),
        ready_to_recv_flag(parent_communicator->get_device().template alloc_memory<ready_to_recv_flag_gpu_type>(1, sizeof(ready_to_recv_flag_gpu_type))),
        local_barrier_flag(parent_communicator->get_device().template alloc_memory<local_barrier_flag_gpu_type>(1, sizeof(local_barrier_flag_gpu_type)))
    {
        LOG_DEBUG(class_name()," entry req ", &req, ", cnt ", elem_count, ", rank: ", comm_addr.to_string());
        size_t next_rank = (comm_addr.rank + 1) % comm_addr.size;
        kernel_router = base::template create_kernel_router_for_rank<l0_allreduce_typed_entry<native_type, gpu_comm_impl, topology>>(*this, next_rank, available_devices);

//TODO L0 workaround
//if(std::is_same<gpu_comm_impl::type_idx() ==, ccl_gpu_comm>::value)
if(gpu_comm_impl::type_idx() == ccl_gpu_comm::type_idx())
{
    wait_count = reinterpret_cast<ccl_gpu_comm*>(parent_communicator.get())->get_virtual_gpu_count() + 1/*+ own*/;
}
else if(gpu_comm_impl::type_idx() == ccl_ipc_source_gpu_comm<ccl_gpu_comm>::type_idx())
{
    wait_count = reinterpret_cast<ccl_ipc_source_gpu_comm<ccl_gpu_comm>*>(parent_communicator.get())->get_impl().get_virtual_gpu_count() + 1/*+ own*/;
}
{
    ...
    void start() override
    {
        ...
        main_entry_function.template set_arg<typename kernel_main_typed::income_data_flag_arg>(income_data_flag.get());
        main_entry_function.template set_arg<typename kernel_main_typed::ready_to_recv_flag_arg>(ready_to_recv_flag.get());
        main_entry_function.template set_arg<typename kernel_main_typed::local_barrier_flag_arg>(local_barrier_flag.get());
...
public:
    bool execute(kernel_main_typed& main_entry_function, kernel_main_typed& right_kernel)
    {
        ...
        bool is_right_kernel_ready = right_kernel.template test_args<typename kernel_main_typed::tmp_recv_buf_arg,
        ...
        if(is_right_kernel_ready)
        {
            ...
            typename kernel_main_typed::ready_to_recv_flag_arg::return_t right_ready_to_recv_flag_arg  =
                                                right_kernel.template get_arg<typename kernel_main_typed::ready_to_recv_flag_arg>();

            LOG_DEBUG("entry: ", class_name(), ", rank: ", comm_addr.to_string(),
                      ", bind elapsed arguments for kernel: ", kernel_main_typed::name());
            ...
            main_entry_function.template set_arg<typename kernel_main_typed::right_tmp_recv_buf_arg>(right_tmp_recv_buf_arg.second);
            LOG_TRACE("Set right_income_data_flag_arg");
            main_entry_function.template set_arg<typename kernel_main_typed::right_income_data_flag_arg>(right_income_data_flag_arg.second);
            LOG_TRACE("Set right_ready_to_recv_flag_arg");
            main_entry_function.template set_arg<typename kernel_main_typed::right_ready_to_recv_flag_arg>(right_ready_to_recv_flag_arg.second);
    ...
    {
        //Check argument binding in kernels for next rank
        bool is_right_kernel_ready = right_kernel.template test_args<typename kernel_ipc_typed::tmp_recv_buf_arg,
        ...
        if(is_right_kernel_ready)
        {
            ...
            typename kernel_main_typed::ready_to_recv_flag_arg::return_t right_ready_to_recv_flag_arg
                                        =   right_kernel.template get_arg<typename kernel_ipc_typed::ready_to_recv_flag_arg>();
