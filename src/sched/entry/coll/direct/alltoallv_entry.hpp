...
public:
    ...
    alltoallv_entry(ccl_sched* sched,
        ...
        send_buf(send_buf), send_cnts(send_cnts),
        recv_buf(recv_buf), recv_cnts(recv_cnts),
        dtype(dtype), comm(comm),
        send_bytes(nullptr), recv_bytes(nullptr),
        send_offsets(nullptr), recv_offsets(nullptr),
        sum_send_bytes(0), sum_recv_bytes(0)
    {
    }
    ...
    void start() override
    {
        ...
        for (i = 1; i < comm_size; i++)
        {
            ...
            send_offsets[i] = send_offsets[i - 1] + send_bytes[i - 1]; // treat buffers as char buffers
        ...
        atl_status_t atl_status = atl_ep_alltoallv(sched->bin->get_atl_ep(),
                                                   send_buf.get_ptr(sum_send_bytes), send_bytes, send_offsets,
                                                   recv_buf.get_ptr(sum_recv_bytes), recv_bytes, recv_offsets,
                                                   &req);
