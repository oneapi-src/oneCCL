                                              ...
                                              ccl_comm* comm)
{
    ...
    entry_factory::make_entry<allgatherv_entry>(sched, send_buf, send_count,
                                             ...
                                             ccl_comm* comm)
{
    ...
    for (size_t rank_idx = 0; rank_idx < comm_size; ++rank_idx)
    {
        if (rank_idx != this_rank)
        {
            ...
            entry_factory::make_chunked_send_entry(sched, recv_buf + offsets[this_rank],
                                                   send_count, dtype, rank_idx, comm);
            // recv other's rank buffer
            entry_factory::make_chunked_recv_entry(sched, recv_buf + offsets[rank_idx],
                                                   recv_counts[rank_idx], dtype, rank_idx, comm);
        }
    }
                                            ...
                                            ccl_comm* comm)
{
    ...
    if (send_buf != recv_buf)
    {
        entry_factory::make_entry<copy_entry>(sched, send_buf, recv_buf + offsets[rank],
                                              send_count, dtype);
    }
